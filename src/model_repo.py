...
class TokenLearner(Module):
    """
    Adapted TokenLearner for 1D input data
    """
    ...
    def forward(self, x):
        # Assuming x is of shape (batch_size, 39)
        return self.net(x)
...
class QRoboticTransformer(Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.action_bins = action_bins
        attend_dim = action_bins
        self.num_learned_tokens = token_learner_num_output_tokens

        # Embedding
        self.embedding_layer = MLPwithEmbedding(state_dim, attend_dim*self.num_learned_tokens, 512)
    ...
    def encode_state(
        ...
        # if state.ndim != 4:
        #    raise ValueError("Dimensions are not matching expected state to have ndim = 4")
        ...
        learned_tokens = self.embedding_layer(state)
        learned_tokens = learned_tokens.view(*state.shape[:-2], num_states*self.num_learned_tokens, self.action_bins)
        #learned_tokens = self.token_learner(tokens)
        #learned_tokens = rearrange(learned_tokens, 'b s c n -> b (s n) c')
